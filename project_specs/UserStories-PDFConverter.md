...
- [ ] Text content, paragraph structure, headings, and lists are preserved in the output DOCX
...
**Acceptance Criteria:**
- [ ] After conversion, the server inspects the output DOCX for text content
- [ ] If the output DOCX contains zero paragraphs with text, the server returns `422` with error code `IMAGE_ONLY_PDF`
...
**As a** Priya Nair (student), **I want** my files to be deleted from the server as soon as I download the DOCX, **so that** I can be confident no copy of my document remains on any third-party server.
...
- [ ] The UI confirms (via success state messaging) that the file has been delivered and is no longer stored
